...
def read_process_memory(pid: int, address: int, size: int = 256) -> str:
    ...
    try:
        ...
        data = bytes(buf[: read.value])  # 只取实际读到的字节
...
def _hexdump(data: bytes, base: int) -> str:
    ...
    for i in range(0, len(data), 16):
        chunk = data[i : i + 16]
        hex_part = " ".join(f"{b:02x}" for b in chunk).ljust(47)  # 凑宽度让右侧 ascii 列对齐
        ascii_part = "".join(chr(b) if 32 <= b < 127 else "." for b in chunk)  # 非可打印字节画点
