...
class _PowerShell:
    ...
    def _ensure(self) -> None:
        ...
        try:
            ...
            logger.warning("shell encoding setup failed", exc_info=True)
    ...
    def _pump(self, proc: subprocess.Popen, output: queue.Queue) -> None:
        ...
        try:
            ...
            logger.warning("shell pump read failed", exc_info=True)
        finally:
...
def _kill_proc(proc: subprocess.Popen) -> None:
    ...
    if pid is not None:
        try:
            ...
            logger.warning("shell taskkill failed for pid %s", pid, exc_info=True)
    try:
        proc.kill()
    except Exception:
        logger.warning("shell proc.kill failed for pid %s", pid, exc_info=True)
    for pipe in (proc.stdout, proc.stdin):
        try:
            if pipe is not None:
                pipe.close()
        except Exception:
            logger.warning("shell pipe close failed", exc_info=True)
    # 后台线程异步收尸
...
def _reap(proc: subprocess.Popen) -> None:
    try:
        ...
        logger.warning("shell reap wait failed", exc_info=True)
...
class _BgShell:
    ...
    def _pump(self) -> None:
        try:
            ...
            logger.warning("bg shell pump read failed", exc_info=True)
        """取上次之后的新输出和漏掉的字符数"""
        with self.lock:
    ...
    def kill(self) -> None:
        ...
        try:
            ...
            logger.warning("bg shell taskkill failed", exc_info=True)
        try:
            self.proc.kill()
        except Exception:
            logger.warning("bg shell proc.kill failed", exc_info=True)
...
def shutdown_background() -> None:
    ...
    for t in tasks:
        try:
            ...
            logger.warning("shutdown_background kill failed", exc_info=True)
