...
class FootprintLayer(QWidget):
    ...
    def add(self, x: int, y: int, heading: float, kind: str = "paw") -> None:
        """落一个脚印"""
        ...
        self._steps.append([x, y, heading, kind, time.time(), self._left])
        if len(self._steps) > _MAX:
            self._steps.pop(0)

    def _tick(self) -> None:
        now = time.time()
        self._steps = [s for s in self._steps if now - s[4] < _LIFE]
        if not self._steps:
            self._timer.stop()
            self.hide()
            return
    ...
    def paintEvent(self, _event) -> None:
        ...
        for x, y, rot, kind, born, left in self._steps:
            age = (now - born) / _LIFE
