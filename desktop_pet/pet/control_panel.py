...
class ControlPanel(QDialog):
    ...
    def __init__(self, settings: Settings, on_reset: Callable[[], None] | None = None,
        ...
        self.setWindowIcon(feixue_icon())
        ...
        side.setSpacing(4)
        avatar = QLabel()
        avatar.setPixmap(feixue_icon().pixmap(QSize(46, 46)))
        avatar.setAlignment(Qt.AlignmentFlag.AlignCenter)
        brand = QLabel("FEIXUE", objectName="brand")
        ...
        for index, (key, builder) in enumerate(page_specs):
            ...
            # key 格式 "tab_xxx" 取 "xxx" 作导航名
            self._nav_buttons[key.removeprefix("tab_")] = btn
    ...
    def _build_fields(self, settings: Settings) -> None:
        ...
        self._context_perch = QCheckBox(self._t("cb_context_perch"))
        self._context_perch.setChecked(getattr(settings, "context_perch_enabled", True))
        self._context_perch.setCursor(Qt.CursorShape.PointingHandCursor)
    ...
    def _refresh_status(self) -> None:
        ...
        stage = self._t("bond_stage_" + str(s.get("bond_stage", "new")))
        self._home_mood.setText(f"{mood} · {stage}")
    ...
    def _build_bond_page(self) -> QWidget:
        ...
        self._bond_recent = QLabel("—")
        body.addWidget(self._section_block(self._t("bond_recent"), self._bond_recent))
        self._bond_next = QLabel("—")
        body.addWidget(self._section_block(self._t("bond_next"), self._bond_next))
    ...
    def _refresh_bond(self) -> None:
        ...
        stage = str(s.get("stage", "new"))
        self._bond_stat.setText(self._t("bond_stage_fmt").format(
            stage=self._t("bond_stage_" + stage), days=s.get("days", 0),
            n=s.get("interactions", 0), sk=s.get("skills", 0), eaten=eaten,
            best=s.get("fishing_best", 0)))
        events = s.get("recent_events") or []
        self._bond_recent.setText(
            "\n".join(f"· {self._t('bond_event_' + event)}" for event in events)
            if events else self._t("bond_recent_empty")
        )
        next_stage = str(s.get("next_stage", ""))
        self._bond_next.setText(
            self._t("bond_next_" + next_stage) if next_stage else self._t("bond_next_max")
        )
    ...
    def _build_connect_page(self) -> QWidget:
        ...
        self._test_btn = QPushButton(i18n.t("connect_test"))
        self._test_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._test_btn.clicked.connect(self._on_test_connection)
        self._test_status = QLabel()
        self._test_status.setWordWrap(True)
        body.addWidget(self._test_btn)
        body.addWidget(self._test_status)
    ...
    def _build_chat_page(self) -> QWidget:
        ...
        body.addWidget(self._check_field(self._context_perch, "help_context_perch"))
    ...
    def _build_about_page(self) -> QWidget:
        ...
        col.setSpacing(7)
        col.addStretch(1)
        col.addWidget(label("FEIXUE", "aboutName"))
        ...
        gh = QLabel(f'<a href="https://github.com/Lucy77m/FEIXUE" style="color: {_ACCENT}; text-decoration: none;">GitHub · Lucy77m/FEIXUE</a>')
    ...
    def _on_gui_model(self) -> None:
        ...
        threading.Thread(target=work, daemon=True, name="feixue-gui-model").start()
    ...
    def _on_check_update(self) -> None:
        ...
        threading.Thread(target=work, daemon=True, name="feixue-update-check-panel").start()
    ...
    def _on_test_connection(self) -> None:
        ...
        self._test_btn.setText(i18n.t("connect_testing"))
        ...
        def _run() -> None:
            ok, key = s.test_connection()
            QTimer.singleShot(0, lambda: self._show_test_result(ok, key))

        threading.Thread(target=_run, daemon=True, name="panel-test-conn").start()

    def _show_test_result(self, ok: bool, key: str) -> None:
        """回到主线程显示测试结果"""
        self._test_btn.setEnabled(True)
        self._test_btn.setText(i18n.t("connect_test"))
        if ok:
            self._test_status.setText(i18n.t("connect_ok"))
            self._test_status.setStyleSheet("color: #2e7d32;")
        elif key == "auth_failed":
            self._test_status.setText(i18n.t("connect_fail_auth"))
            self._test_status.setStyleSheet("color: #c62828;")
        elif key in ("not_found", "ConnectError", "ConnectTimeout"):
            self._test_status.setText(i18n.t("connect_fail_url"))
            self._test_status.setStyleSheet("color: #c62828;")
        else:
            self._test_status.setText(i18n.t("connect_fail_generic").format(error=key))
            self._test_status.setStyleSheet("color: #c62828;")
    ...
    def _on_save(self) -> None:
        ...
        errors = Settings(
            ...
            base_url=self._base_url.text().strip(),
            model=self._model.text().strip(),
            history_tokens=int(self._history_tokens.text() or 24000),
        ).validate()
        _RED = "border: 1px solid #c62828;"
        _CLEAR = ""
        self._api_key.setStyleSheet(_RED if "api_key_empty" in errors else _CLEAR)
        self._base_url.setStyleSheet(_RED if any(e.startswith("base_url") for e in errors) else _CLEAR)
        self._model.setStyleSheet(_RED if "model_empty" in errors else _CLEAR)
        # --- 正常保存流程 ---
        s = self._settings
        ...
        s.context_perch_enabled = self._context_perch.isChecked()
    ...
    def navigate_to(self, page_name: str) -> None:
        """切换到指定设置页。page_name: 'home', 'bond', 'docs', 'connect', 'chat', 'interact', 'perm', 'about'"""
