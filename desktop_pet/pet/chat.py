...
def attachment_payload(paths: list[str]) -> list[dict]:
    payload = []
    for raw in paths:
        path = Path(raw)
        if not path.is_file():
        ...
        if path.suffix.lower() in _IMAGE_EXT:
            ...
            if not image.isNull():
                payload.append({"kind": "image", "data_url": _image_to_data_url(image), "name": path.name})
                continue
        payload.append({"kind": "file", "path": str(path), "name": path.name})
    return payload
_ATTACH_MAX = 8
_IMG_MAX_SIDE = 1568
...
class InputBox(QWidget):
    ...
    def draft(self, text: str, pet: QWidget) -> None:
        self._editor.setPlainText(text)
