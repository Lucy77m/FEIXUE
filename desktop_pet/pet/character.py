...
from desktop_pet.pet.sprite_skin import SpritePetController


class BlobPet(ThinkMixin, ReactFxMixin, FaceMixin):
    """桌宠形象本体 状态机加绘制"""

    def __init__(self, skin_name: str = "blob") -> None:
        skin_name = (skin_name or "blob").strip()
        self._sprite = SpritePetController(skin_name) if skin_name != "blob" else None
        self._t = 0.0
        self._expr = "neutral"
    ...
    @property
    def has_sprite_skin(self) -> bool:
        return self._sprite is not None and self._sprite.available

    def visual_anchors(self, w: int, h: int) -> dict[str, QPointF]:
        if self.has_sprite_skin:
            return self._sprite.visual_anchors(w, h)
        cx, cy = w / 2, h / 2
        return {
            "foot": QPointF(cx, cy + BLOB_HALF_H),
            "head": QPointF(cx + BLOB_HALF_W * 0.6, cy - BLOB_HALF_H),
            "head_top": QPointF(cx, cy - BLOB_HALF_H),
        }
    ...
    def is_busy(self) -> bool:
        return self._busy or self._lecturing or self._pondering

    @property
    def can_sprite_wander(self) -> bool:
        return (
            self.has_sprite_skin
            and not self._talking
            and not self._busy
            and not self._lecturing
            and not self._dragging
            and not self._hidden
            and not self._asleep
            and not self._falling_asleep
            and self._react is None
            and self._activity is None
            and self._pending_perform is None
        )

    def notice_attention(self) -> bool:
        if self._sprite is None:
            return False
        return self._sprite.notice()

    def set_sprite_walking(self, on: bool, direction: int = 0) -> None:
        if self._sprite is not None:
            self._sprite.set_walking(on, direction)

    def set_busy(self, busy: bool) -> None:
        # busy时强制思考脸并清掉发呆反应
        self._busy = busy
        if self._sprite is not None:
            self._sprite.set_busy(busy)
        if busy:
            self._expr = "thinking"
            self._hold = 0.0
            self._react = None
            ...
            if self._activity is None:
                # 清掉上一条回复留下的临时戏服 _hold 清零后衰减清除路径不再跑 否则戏服一直挂着
    ...
    def set_dragging(self, on: bool) -> None:
        self._dragging = on
        if self._sprite is not None:
            self._sprite.set_dragging(on)

    def set_hidden(self, on: bool) -> None:
        self._hidden = on
        if self._sprite is not None:
            self._sprite.set_hidden(on)
    ...
    def start_activity(self, name: str) -> bool:
        ...
        self._activity_timer = random.uniform(*_ACTIVITY_GAP)
        if self._sprite is not None:
            self._sprite.start_activity(name)
        return True
    ...
    def advance(self, dt: float) -> bool:
        ...
        if self._sprite is not None:
            sprite_changed = self._sprite.advance(dt)
        ...
        # 只有真正演出时环境装饰才平滑退场 小品 反应 拖拽 藏起 睡着这几种算
        # 说话和思考不算 否则伞之类的持续装饰会随每句话一闪一闪
        if (self._dragging or self._hidden or self._react is not None
                or self._activity is not None or self.is_asleep):
        ...
        if self._react:
            ...
            return sprite_changed or not self.has_sprite_skin
        if self._settle > 0:
    ...
    def _advance_activity(self, dt: float) -> None:
        ...
        if self._activity is not None:
            ...
            # 卡死兜底 演太久才强制收场 防 blob 缩没卡住 阈值取自身阶段总时长加 5s 余量
            ...
            if self._activity_timer <= 0.0:
                ...
                if idle:
                    ...
                    if name == _TRAVEL:
                        ...
                        self._activity = name
                        self._activity_age = 0.0
                        self._act_sticky = False  # 自发小品 软打断 说话思考 就让位
                        self._costume = _ACTIVITIES[name][0]
                        self._stage_i = 0
                        self._enter_stage(_ACTIVITIES[name][3][0])
                        if self._sprite is not None:
    ...
    def paint(self, painter: QPainter, w: int, h: int) -> None:
        ...
        self._win_h = float(h)
        if self.has_sprite_skin:
            self._sprite.paint(painter, w, h)
            return
