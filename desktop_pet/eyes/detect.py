...
_MODEL_URL = "https://github.com/Lucy77m/FEIXUE/releases/download/models/ui_detect.onnx"
_MODEL_SHA256 = ""  # Set to the expected SHA256 hex digest; empty string skips verification.
...
def _verify_checksum(path: Path) -> str:
    ...
    Skips verification if _MODEL_SHA256 is empty.
    """
    if not _MODEL_SHA256:
        return ""
    h = hashlib.sha256()
        ...
        try:
            with client.stream("GET", _MODEL_URL, follow_redirects=True) as r:
                ...
                if cancelled:
                    ...
                    return "cancelled"
                tmp.replace(target)
                err = _verify_checksum(target)
                if err:
                    return f"download ok but {err}"
        finally:
            client.close()
