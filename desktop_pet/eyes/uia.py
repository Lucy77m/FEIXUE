...
def _pump() -> None:
    ...
    try:
        ...
        logger.debug("uia: CoInitializeEx failed", exc_info=True)
    while True:
...
def _ensure_thread() -> None:
    ...
    with _thread_lock:
        if _thread is None or not _thread.is_alive():
            t = threading.Thread(target=_pump, name="feixue-uia", daemon=True)
            t.start()
            _thread = t
...
def _window(title: str | None):
    ...
    if not title:
        try:
            ...
            logger.warning("uia: GetForegroundControl failed", exc_info=True)
            return None
    try:
        ...
        logger.warning("uia: GetRootControl iteration failed", exc_info=True)
    return None
    ...
    while q and seen < _MAX_NODES:
        ...
        if depth < _MAX_DEPTH:
            try:
                ...
                logger.debug("uia: walk child enumeration failed", exc_info=True)
...
def _sync_geom(win) -> None:
    ...
    try:
        ...
        logger.debug("uia: sync_geom failed", exc_info=True)
...
def _kind(ctrl) -> str:
    try:
        ...
        logger.debug("uia: kind read failed", exc_info=True)
        return ""
...
def _read_value(ctrl, kind: str) -> str:
    ...
    try:
        ...
        logger.debug("uia: read_value failed", exc_info=True)
    return ""
...
def _do_set_value(ctrl, text: str) -> bool:
    try:
        ...
        logger.debug("uia: set_value failed", exc_info=True)
    return False
...
def _do_scroll_into_view(ctrl) -> bool:
    ...
    try:
        ...
        logger.debug("uia: scroll_into_view pattern failed", exc_info=True)
    try:
        ctrl.SetFocus()
        return True
    except Exception:
        logger.debug("uia: scroll_into_view set_focus failed", exc_info=True)
    return False
