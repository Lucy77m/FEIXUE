...
class FeedingCtrl(QObject):
    ...
    def __init__(self, host) -> None:
        ...
        self._host._pet.fed.connect(self.offer)
    ...
    @Slot(list)
    def offer(self, paths: list) -> bool:
        """投喂入口 按类型分流"""
        if self._stopped or not paths:
            return False
        ...
        self._sizing = True
        threading.Thread(target=self._size_then_decide, args=(paths,), daemon=True, name="feixue-feed-size").start()
        return True
    ...
    def _finish_eat(self, paths: list, total: int) -> None:
        ...
        threading.Thread(target=work, daemon=True, name="feixue-feed-eat").start()
