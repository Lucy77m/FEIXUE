...
class Sensors(QObject):
    ...
    def _shy_probe_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: shy probe failed", exc_info=True)
        finally:
            self._shy_checking = False
    ...
    def _vitals_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: vitals read failed", exc_info=True)
        finally:
            self._vitals_busy = False
    ...
    def _dl_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: download scan failed", exc_info=True)
        finally:
            self._dl_busy = False
    ...
    def _mic_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: mic check failed", exc_info=True)
        finally:
            self._mic_busy = False
    ...
    def _desk_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: desktop scan failed", exc_info=True)
        finally:
            self._desk_busy = False
    ...
    def _weather_thread(self) -> None:
        try:
            ...
            logger.debug("sensors: weather check failed", exc_info=True)
        finally:
            self._weather_busy = False
    ...
    def _on_weather(self, kind: str) -> None:
        ...
        # 天气避难行为
        if kind == "rain":
            QTimer.singleShot(3000, lambda: self._host._feed_perform("read"))
            self._host._feed_pop(i18n.t("weather_rain_shelter"))
        elif kind == "snow":
            QTimer.singleShot(2000, lambda: self._host._feed_react("happy_wiggle"))
            self._host._feed_pop(i18n.t("weather_snow_footprints"))
