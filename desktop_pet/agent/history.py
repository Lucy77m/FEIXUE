...
class HistoryMixin:
    ...
    def _queue_compression(self, dropped: list[dict]) -> None:
        ...
        try:
            threading.Thread(target=self._compress_worker, daemon=True, name="feixue-compress").start()
        except RuntimeError:
    ...
    def _ingest_and_read(self, files: list) -> str:
        ...
        for f in files:
            ...
            if not path:
                continue
            try:
                threading.Thread(target=self._safe_ingest, args=(path,), daemon=True, name="feixue-ingest").start()
            except RuntimeError:
                logger.debug("ingest thread launch failed for %s", path)
    ...
    def _safe_ingest(path: str) -> None:
        try:
            ...
            logger.debug("safe_ingest failed for %s", path, exc_info=True)
