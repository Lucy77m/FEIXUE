...
# mcp连接中枢 后台线程连stdio服务 汇总schema并转发调用
...
def _render_result(result) -> str:
    """mcp回包压成纯文本 非文本块退化成占位符"""
    parts: list[str] = []
    for item in getattr(result, "content", None) or []:
        text = getattr(item, "text", None)
        # getattr兜底字段不齐的server
...
class MCPHub:
    ...
    async def _serve(self, servers: dict[str, dict]) -> None:
        """逐个连server点亮ready后等关停"""
        # mcp包延迟import 没装也能正常用
    ...
    def shutdown(self) -> None:
        ...
        if self._thread is not None:
            self._thread.join(timeout=5.0)  # 等不到就放手别卡死退出
