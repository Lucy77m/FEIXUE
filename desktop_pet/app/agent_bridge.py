...
class AgentBridgeMixin:
    ...
    def _on_submit(self, _text: str, _attachments: object = None) -> None:
        try:
            ...
            logger.debug("agent_bridge: emotion apply failed", exc_info=True)
    ...
    def _on_proactive_reply(self, raw: str) -> None:
        ...
        if workshop is not None and workshop.accept_revisit_reply(raw):
            return
    ...
    def _on_reply(self, raw: str) -> None:
        ...
        if wf is not None:
            wf.on_reply(raw)
        ws = getattr(self, "_workshop", None)
        if ws is not None and ws.is_active():
            return
        if not self._shown:
    ...
    def _engaged(self) -> bool:
        """是否正忙或用户交互中"""
        fishing = getattr(self, "_fishing", None)
        workshop = getattr(self, "_workshop", None)
        return (self._worker.is_running or self._busy or self._lecturing
                or self._speech.is_speaking or self._input.isVisible()
                or (fishing is not None and fishing.is_active())
                or (workshop is not None and workshop.is_active()))
    ...
    def _on_manual_perform(self, name: str) -> None:
        """User menu commands stay available after cancelling an agent turn."""
        self._wake()
        self._pet.perform(name)
    ...
    def _on_busy(self, busy: bool) -> None:
        ...
        if busy:
            ...
            if not (getattr(self, "_workshop", None) and self._workshop.is_active()):
    ...
    def _on_task_finished(self, ok: bool) -> None:
        ws = getattr(self, "_workshop", None)
        workshop_active = ws is not None and ws.is_active()
        context = self._playtime.context_kind or self._proactive_context()
        wf = getattr(self, "_workflow", None)
        if wf is not None:
            wf.complete(ok, context=context)
        try:
            emotion.apply("task_done" if ok else "task_failed")
        except Exception:
            logger.debug("agent_bridge: task_done emotion failed", exc_info=True)
        selector.set_emotion(*emotion.snapshot())
        if workshop_active:
            return
