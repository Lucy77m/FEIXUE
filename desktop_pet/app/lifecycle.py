...
from desktop_pet import hearing, i18n, journal, keepsakes, persona, stats
...
class LifecycleMixin:
    ...
    def _status_snapshot(self) -> dict:
        ...
        return {
            ...
            "rapport": bond["rapport"],
            "bond_stage": bond["stage"],
    ...
    def _bond_snapshot(self) -> dict:
        ...
        return {
            ...
            **bond,
            ...
            "fishing_best": st.get("fishing_best", 0),
        }

    def _on_bond_stage_unlocked(self, stage: str) -> None:
        if stage == "new":
            return
        journal.add(i18n.t("bond_stage_journal").format(stage=i18n.t("bond_stage_" + stage)))
        self._pet.react("celebrate")
        QTimer.singleShot(
            650,
            lambda: self._feed_pop(i18n.t("bond_stage_unlocked").format(
                stage=i18n.t("bond_stage_" + stage))),
        )
    ...
    def _open_panel(self) -> None:
        ...
        self._panel.raise_()
        self._panel.activateWindow()
        # 首次未配置时自动跳转到连接页面并显示引导提示
        if not self._settings.is_configured:
            self._panel.navigate_to("connect")
            self._feed_pop(i18n.t("hint_first_setup"))
    ...
    def _apply_settings_live(self) -> None:
        ...
        try:
            ...
            logger.debug("lifecycle: weather check failed", exc_info=True)
    ...
    def _quit(self) -> None:
        ...
        try:
            ...
            logger.debug("lifecycle: tray hide failed", exc_info=True)
        if self._panel is not None:
            try:
                self._panel.reject()
            except Exception:
                logger.debug("lifecycle: panel reject failed", exc_info=True)
            return
        self._do_quit()
    ...
    def _do_quit(self) -> None:
        ...
        for c in (self._sensors, self._playtime, self._watchers, self._rituals,
            ...
            try:
                ...
                logger.warning("lifecycle: companion %s stop failed", type(c).__name__, exc_info=True)
        ...
        for _t in (self._presence_timer, self._reminder_timer, self._proactive_timer,
            ...
            try:
                ...
                logger.debug("lifecycle: timer stop failed", exc_info=True)
        if self._rituals.farewell():
            return
        self._keepsake_shelf.close()
        self._requeue_timed()  # 没派发的定时任务写回 reminders 持久化 别随 os._exit 一起丢掉
        self._hotkeys.stop()
        try:
            hearing.shutdown()
        except Exception:
            logger.warning("lifecycle: hearing shutdown failed", exc_info=True)
        ...
        try:
            ...
            logger.debug("lifecycle: bgtasks shutdown failed", exc_info=True)
        ...
        try:
            ...
            logger.debug("lifecycle: shell shutdown_background failed", exc_info=True)
        try:
            mcp_hub.shutdown()
        except Exception:
            logger.debug("lifecycle: mcp_hub shutdown failed", exc_info=True)
        # 硬退之前干净关掉两个 SQLite 库 等后台反思的在途 commit 收尾再关 否则退出会把库截断成 malformed
        for db in (store, docs):
            try:
                db.close()
            except Exception:
                logger.warning("lifecycle: db close failed for %s", type(db).__name__, exc_info=True)
