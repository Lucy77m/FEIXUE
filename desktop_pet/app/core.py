...
from desktop_pet.companions.boredom import Boredom
...
class PetApp(QuickActionsMixin, VoiceMixin, AgentBridgeMixin,
    ...
    def __init__(self) -> None:
        ...
        self._app.setWindowIcon(feixue_icon())
        super().__init__()
        ...
        self._pet = PetWindow(self._settings.pet_skin, self._settings.pet_scale)
        ...
        self._boredom = Boredom(self)
        self._workshop = WorkshopCtrl(self)
        self._workflow = WorkflowCtrl(self)
        ...
        self._tray = Tray(
            ...
            on_fishing=self._fishing.start,
            on_workshop=self._open_workshop,
    ...
    def _connect(self) -> None:
        ...
        self._pet.scale_changed.connect(self._on_pet_scale_changed)
    ...
    def _toggle_input(self) -> None:
        self._wake()
        if self._workshop.consume_pending_revisit():
            return
    ...
    @Slot()
    def _open_keepsakes(self) -> None:
        self._open_keepsake("")

    @Slot()
    def _open_history(self) -> None:
        if self._history_viewer is None:
            from desktop_pet.pet.history_viewer import HistoryViewer
    ...
    def _open_keepsake(self, item_id: str) -> None:
        screen = (self._app.screenAt(self._pet.frameGeometry().center())
                  or self._app.primaryScreen()).availableGeometry()
        self._keepsake_shelf.popup(self._pet, screen, item_id)

    @Slot(str)
    def _resume_keepsake(self, prompt: str) -> None:
        if not self._shown:
            self._power_on()
        self._pet.wake()
        self._input.draft(prompt, self._pet)

    @Slot(int)
    def _on_pet_scale_changed(self, scale: int) -> None:
        self._settings.pet_scale = scale
        self._settings.save()
