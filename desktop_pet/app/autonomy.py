...
class AutonomyMixin:
    ...
    def _on_presence(self) -> None:
        try:
            ...
            logger.debug("autonomy: on_presence failed", exc_info=True)
    ...
    def _check_reminders(self) -> None:
        try:
            ...
            logger.debug("autonomy: check_reminders failed", exc_info=True)
    ...
    def _foreground_is_fullscreen() -> bool:
        try:
            ...
            logger.debug("autonomy: fullscreen check failed", exc_info=True)
            return False
    ...
    def _requeue_timed(self) -> None:
        ...
        for task in pending:
            try:
                ...
                logger.debug("autonomy: requeue timed task failed", exc_info=True)
    ...
    def _check_proactive(self) -> None:
        try:
            ...
            logger.debug("autonomy: check_proactive failed", exc_info=True)
    ...
    def _check_watch(self) -> None:
        try:
            ...
            logger.debug("autonomy: check_watch failed", exc_info=True)
    ...
    def _maybe_speak_up(self) -> None:
        ...
        if self._settings.allow_web and random.random() < _EXPLORE_CHANCE:
            ...
            mode = self._pick_proactive_mode(now)
            if mode == "daily_summary":
                context = self._build_daily_summary_context()
            self.request_proactive.emit(mode, context)
    ...
    @staticmethod
    def _build_daily_summary_context() -> str:
        """组装每日工作总结的丰富上下文。"""
        from desktop_pet import journal, stats as _stats
        from desktop_pet.emotion.state import emotion
        parts = []
        journal_ctx = journal.as_context(n=10)
        if journal_ctx:
            parts.append(journal_ctx)
        snap = _stats.snapshot()
        parts.append(f"（你们已经认识 {snap['days']} 天，互动了 {snap['interactions']} 次。）")
        val, _aro, _rap = emotion.snapshot()
        mood = "开心" if val >= 0.25 else "平静" if val >= -0.15 else "有点低落"
        parts.append(f"（你今天整体感觉{mood}。）")
        _stats.set_note("daily_summary", datetime.now().strftime("%Y-%m-%d"))
        return "\n".join(parts)
    ...
    def _pick_proactive_mode(now: datetime) -> str:
        from desktop_pet import stats
        # 傍晚每日总结：17:00-21:00，每天最多一次
        if 17 <= now.hour < 21:
            if stats.get_note("daily_summary") != now.strftime("%Y-%m-%d"):
