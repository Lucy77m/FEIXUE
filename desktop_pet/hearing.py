...
_DL_BASE = "https://github.com/Lucy77m/FEIXUE/releases/download/models"
...
_capturing_evt = threading.Event()  # 正在采集一句 防热键重入
...
def _app_busy() -> bool:
    try:
        ...
        logger.debug("hearing: _app_busy callback failed", exc_info=True)
        return False
...
def _wake_blocked() -> bool:
    try:
        ...
        logger.debug("hearing: _wake_blocked callback failed", exc_info=True)
        return False
...
def start_download(proxy: str = "") -> None:
    ...
    if is_ready():
        ...
        return
    threading.Thread(target=_download, args=(proxy,), daemon=True, name="feixue-hear-dl").start()
    ...
    try:
        ...
        for key in _FILES:
            ...
            try:
                ...
                logger.debug("hearing: download part cleanup failed", exc_info=True)
        with _dl_lock:
...
def _warmup_async() -> None:
    ...
    def _go():
        ...
        try:
            ...
            logger.debug("hearing: warmup failed", exc_info=True)
        finally:
            with _model_lock:
                _warming = False

    threading.Thread(target=_go, daemon=True, name="feixue-hear-warmup").start()


def start_talk() -> None:
    """热键按下开始听 采集中重按或正忙直接忽略"""
    if not _enabled or _capturing_evt.is_set() or _app_busy():
        return
    _talk_end.clear()
    _talk_req.set()
    _kick()
...
def _kick() -> None:
    global _loop_thread
    with _model_lock:
        enabled_or_wake = _enabled or _wake_on
    if not enabled_or_wake:
        return
    with _lock:
        if _loop_thread is not None and _loop_thread.is_alive():
            return
        _loop_thread = threading.Thread(target=_loop, daemon=True, name="feixue-hearing")
        _loop_thread.start()
...
def _import_sherpa():
    ...
    if os.path.exists(dll):
        try:
            ...
            logger.debug("hearing: onnxruntime DLL preload failed", exc_info=True)
    import sherpa_onnx
    return sherpa_onnx
...
def _emit(cb, *args) -> None:
    if cb is not None:
        try:
            ...
            logger.debug("hearing: callback %s failed", getattr(cb, '__name__', cb), exc_info=True)
...
def _loop() -> None:
    ...
    try:
        ...
        logger.debug("hearing: SetThreadPriority failed", exc_info=True)
    stream = None
    ks = None
    try:
        def _on_audio(indata, frames, t, status):
            try:
                ...
                logger.debug("hearing: audio queue put failed", exc_info=True)

        last_audio = time.monotonic()
        while not _shutdown.is_set():
            with _model_lock:
                loop_enabled, loop_wake_on = _enabled, _wake_on
            if not (loop_enabled or loop_wake_on):
                break  # 全关了线程退出释放麦克风
            if stream is None:
            ...
            while not _shutdown.is_set():
                if talking_src == "hotkey" and _talk_end.is_set():
                    break
                if (talking_src == "hotkey" and not loop_enabled) or (talking_src == "wake" and not loop_wake_on):
                ...
                if buf and now - last_partial >= _PARTIAL_EVERY:
                    ...
                    try:
                        ...
                        logger.debug("hearing: partial decode failed", exc_info=True)
            if _shutdown.is_set():
        ...
        if stream is not None:
            try:
                ...
                logger.debug("hearing: stream close failed", exc_info=True)
        _drain(_audio_q)
        global _loop_thread
        _capturing_evt.clear()  # 异常中途退出也不卡死下一次按键
        with _lock:
            _loop_thread = None
        ...
        if not _shutdown.is_set():
            with _model_lock:
                loop_enabled, loop_wake_on = _enabled, _wake_on
            if loop_enabled or loop_wake_on:
                _kick()
...
def _check_stall(stream, last_audio: float):
    ...
    if stream is not None and time.monotonic() - last_audio > _STALL_S:
        try:
            ...
            logger.debug("hearing: stall stream close failed", exc_info=True)
        return None
    return stream
